...
router.get('/hod/:hodId', async (req, res) => {
  try {
    // First get the HOD name from the hods table
    const [hodRows] = await db.query('SELECT name FROM hods WHERE id = ?', [req.params.hodId]);
    ...
    // Get schemes using the hod name (since schemes table uses hod varchar, not hod_id)
    const [results] = await db.query(`
      SELECT s.*, 
             COALESCE(SUM(sba.allocated_amount), 0) as budget_allocated,
             COALESCE(SUM(sba.spent_amount), 0) as budget_utilized
