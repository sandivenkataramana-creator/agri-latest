...
const KPIs = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'on_track':
        return <FiCheckCircle className="status-icon icon-on-track" />;
      case 'at_risk':
        return <FiAlertCircle className="status-icon icon-at-risk" />;
      case 'behind':
        return <FiAlertCircle className="status-icon icon-behind" />;
      case 'completed':
        return <FiCheckCircle className="status-icon icon-completed" />;
      default:
        return null;
    }
  };
        ...
        {kpis.map((kpi) => {
          ...
          return (
            <div className="widget-card" key={kpi.id}>
              <div className={`widget-header header-${kpi.status}`}>
                <h4>{kpi.kpi_name}</h4>
              ...
              <div className="widget-body">
                ...
                <div className="progress-bar progress-margin">
                  <progress
                    max="100"
                    value={Math.min(progress, 100)}
                    className={`progress-meter ${kpi.status === 'on_track' ? 'green' : kpi.status === 'at_risk' ? 'orange' : 'blue'}`}
                  />
                </div>
