    ...
    <div className="page-container">
      ...
      {departmentData && departmentData.length > 0 && (
        <div className="department-grid-section">
          <h3 className="dept-section-title">Department-wise Attendance Overview</h3>
          <div className="department-grid-container">
            {departmentData.map((dept, deptIndex) => (
              <div key={deptIndex} className="dept-overview-card">
                {/* Header */}
                <div className="dept-overview-header">
                  <h4 className="dept-overview-title">{dept.department}</h4>
                  <span className="dept-total-badge">{dept.total_emp || 0} Staff</span>
                </div>
                ...
                <div className="dept-metrics-grid">
                  ...
                  <div 
                    ...
                    className="dept-metric total"
                  >
                    <div className="metric-icon"><FiUsers size={16} /></div>
                    <div className="metric-label">Total</div>
                    <div className="metric-value">{dept.total_emp || 0}</div>
                  </div>

                  {/* Present */}
                  <div 
                    onClick={() => handleDepartmentStatusPopup(dept.department, dept.department, 'present', 'Present', dept.employees?.present || [])}
                    className="dept-metric present"
                  >
                    <div className="metric-icon"><FiCheckCircle size={16} /></div>
                    <div className="metric-label">Present</div>
                    <div className="metric-value">{dept.present || 0}</div>
                    <div className="metric-percent">{dept.total_emp > 0 ? ((dept.present / dept.total_emp) * 100).toFixed(0) : 0}%</div>
                  </div>

                  {/* Absent */}
                  <div 
                    onClick={() => handleDepartmentStatusPopup(dept.department, dept.department, 'absent', 'Absent', dept.employees.absent)}
                    className="dept-metric absent"
                  >
                    <div className="metric-icon"><FiXCircle size={16} /></div>
                    <div className="metric-label">Absent</div>
                    <div className="metric-value">{dept.absent || 0}</div>
                    <div className="metric-percent">{dept.total_emp > 0 ? ((dept.absent / dept.total_emp) * 100).toFixed(0) : 0}%</div>
                  </div>

                  {/* Late */}
                  <div 
                    onClick={() => handleDepartmentStatusPopup(dept.department, dept.department, 'late', 'Late (after 10:30)', dept.employees.late)}
                    className="dept-metric late"
                  >
                    <div className="metric-icon"><FiClock size={16} /></div>
                    <div className="metric-label">Late</div>
                    <div className="metric-value">{dept.late || 0}</div>
                    <div className="metric-percent">{dept.total_emp > 0 ? ((dept.late / dept.total_emp) * 100).toFixed(0) : 0}%</div>
                  </div>

                  {/* On Leave */}
                  <div 
                    onClick={() => handleDepartmentStatusPopup(dept.department, dept.department, 'leave', 'Emp Leave', dept.employees.leave)}
                    className="dept-metric leave"
                  >
                    <div className="metric-icon"><FiCalendar size={16} /></div>
                    <div className="metric-label">Leave</div>
                    <div className="metric-value">{dept.emp_leave || 0}</div>
                    <div className="metric-percent">{dept.total_emp > 0 ? ((dept.emp_leave / dept.total_emp) * 100).toFixed(0) : 0}%</div>
                  </div>

                  {/* Attendance % */}
                  <div className="dept-metric attendance-percent">
                    <div className="metric-icon"><FiFilter size={16} /></div>
                    <div className="metric-label">Attendance</div>
                    <div className="metric-value" style={{
                      color: (dept.total_emp > 0 && ((dept.present / dept.total_emp) * 100) >= 90) ? '#4CAF50' : 
                             (dept.total_emp > 0 && ((dept.present / dept.total_emp) * 100) >= 75) ? '#FF9800' : '#F44336'
                    }}>
                      {dept.total_emp > 0 ? ((dept.present / dept.total_emp) * 100).toFixed(0) : 0}%
                    </div>
                  </div>
                </div>
              </div>
