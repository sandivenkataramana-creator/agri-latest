...
const Schemes = () => {
  ...
  // Fetch available financial years from database
  useEffect(() => {
    const fetchAvailableYears = async () => {
      try {
        const response = await fetch(`${appConfig.apiBaseUrl}/schemes/available-years`);
        const years = await response.json();
        setAvailableYears(years);
        // Set first available year if exists and not already set
        if (years.length > 0 && financialYear === '2025-26') {
          setFinancialYear(years[0]);
        }
      } catch (err) {
        console.error('Error fetching available years:', err);
        setAvailableYears(['2025-26']); // Fallback
      }
    };
    fetchAvailableYears();
  }, []);
  ...
  useEffect(() => {
    // Fetch all data types on initial load
    if (financialYear) {
      fetchCentralSchemes();
  ...
  const fetchData = async () => {
    try {
      ...
      const [hodsRes, categoriesRes, financialRes] = await Promise.all([
        ...
        getCategories(),
        fetch(`${appConfig.apiBaseUrl}/schemes/financial-progress?year=${financialYear}&_t=${Date.now()}`) // Add cache-buster
          .then(res => res.json())
          .catch(err => {
      ...
      setFinancialRows(sortByName(financialData, 'central_scheme_name', 'asc'));
  ...
  const fetchStateSchemeData = async () => {
    try {
      const response = await fetch(`${appConfig.apiBaseUrl}/schemes/state-schemes/all?year=${financialYear}&_t=${Date.now()}`);
      const data = await response.json();
     const sorted = sortByName(data, 'name', 'asc');
setStateSchemeData(sorted);
  ...
  const fetchRevenueData = async () => {
    try {
      const response = await fetch(`${appConfig.apiBaseUrl}/schemes/revenue/all?year=${financialYear}&_t=${Date.now()}`);
      const data = await response.json();
      const sorted = sortByName(data, 'cooperativeName', 'asc');
...
const fetchCentralSchemes = async (year = financialYear) => {
  try {
    ...
    const res = await fetch(
      `${appConfig.apiBaseUrl}/schemes/financial-progress?year=${year}&_t=${Date.now()}`
    );
    const data = await res.json();

    console.log('Central schemes refreshed:', data.length);
   const sorted = sortByName(data, 'central_scheme_name', 'asc');
setFinancialRows(sorted);
...
const sortByName = (data, key, order = 'asc') => {
  ...
  const handleDeleteStateScheme = async (row) => {
    ...
    if (!ok) return;
    try {
      const response = await fetch(`${appConfig.apiBaseUrl}/schemes/state-schemes/${row.id}`, {
        method: 'DELETE'
      });
      if (!response.ok) {
  ...
  const handleDeleteRevenueEntry = async (row) => {
    ...
    if (!ok) return;
    try {
      const response = await fetch(`${appConfig.apiBaseUrl}/schemes/revenue/${row.id}`, {
        method: 'DELETE'
      });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // STATE SCHEME SUBMISSION
        ...
        const url = isEditing
          ? `${appConfig.apiBaseUrl}/schemes/state-schemes/${editingItem.id}`
          : `${appConfig.apiBaseUrl}/schemes/state-schemes`;
      ...
      // REVENUE SUBMISSION
        ...
        const url = isEditing
          ? `${appConfig.apiBaseUrl}/schemes/revenue/${editingItem.id}`
          : `${appConfig.apiBaseUrl}/schemes/revenue`;
  ...
  const parseNumberOrNull = (value) => {
    ...
    // Convert strings like "1,234.50", "Γé╣ 12.5", "12 Cr"
...
const parseExcelDate = (value) => {
  ...
  const handleCentralSchemeImport = async (file) => {
    try {
      ...
      const existingResponse = await fetch(`${appConfig.apiBaseUrl}/schemes/financial-progress?year=${financialYear}`);
        ...
        try {
          ...
          for (const [index, row] of jsonData.entries()) {
            ...
            try {
              ...
              // Γ£à AUTO-CALCULATE TOTALS IF MISSING
if (
...
if (
  ...
  schemeData.sanction_total =
    schemeData.sanction_goi_share + schemeData.sanction_state_share;
              ...
              // Γ£à DEBUG: detect missing allocation_total
              if (schemeData.allocation_total === null) {
                console.warn('ΓÜá∩╕Å Allocation total missing', {
              ...
              if (schemeData.scheme_name && schemeData.hod) {
                try {
                  ...
                  if (!existingScheme) {
                    ...
                    if (existingScheme) {
                      console.log('Γ£ô Found via fuzzy match:', { newName: schemeData.scheme_name, existingId: existingScheme.id });
                    }
                  }
                  ...
                  if (!existingScheme && schemeData.scheme_name?.length > 20) {
                    ...
                    if (existingScheme) {
                      console.log('Γ£ô Found via partial match:', { newName: schemeData.scheme_name, existingId: existingScheme.id });
                    }
                  }
                  
                  if (existingScheme && existingScheme.id) {
                    // Update existing scheme
                    console.log('≡ƒô¥ Updating existing scheme', { id: existingScheme.id, scheme: schemeData.scheme_name });
                    await updateScheme(existingScheme.id, schemeData);
                    successCount++;
                    console.log('Γ£à Row updated successfully', { index: index + 1, scheme: schemeData.scheme_name, id: existingScheme.id });
                  } else {
                    // Create new scheme
                    console.log('Γ₧ò Creating new scheme', { scheme: schemeData.scheme_name });
                    await createScheme(schemeData);
                    successCount++;
                    console.log('Γ£à Row imported successfully', { index: index + 1, scheme: schemeData.scheme_name });
                  }
                } catch (apiErr) {
                  errorCount++;
          ...
          //       console.log('Γ£à Setting financialRows with', freshData.length, 'records');
          ...
          //         console.log('Γ£à Setting financialRows on retry with', retryData.length, 'records');
          //         setFinancialRows(Array.isArray(retryData) ? retryData : []);
          //       }, 1000);
          //     }
          //   } catch (err) {
          //     console.error('Γ¥î Error refreshing data:', err);
...
const handleStateSchemeImport = async (file) => {
  ...
  reader.onload = async (event) => {
    try {
      ...
      // Γ£à READ AS ARRAY (NOT OBJECT)
      const jsonData = XLSX.utils.sheet_to_json(worksheet, {
        header: 1,
        defval: null
      });

      // Γ£à Remove header row
      jsonData.shift();
...
const handleRevenueImport = async (file) => {
        ...
        for (const [index, row] of jsonData.entries()) {
          ...
          try {
            ...
            if (revenueData.cooperativeName) {
              const response = await fetch(`${appConfig.apiBaseUrl}/schemes/revenue`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
              ...
              if (response.ok) {
                successCount++;
                console.log('Γ£à Revenue row imported:', { index: index + 1, name: revenueData.cooperativeName });
              } else {
                errorCount++;
  ...
  if (filterType === 'central-sponsored-scheme') {
    headers = [
      ...
      'HOD',
      'Financial Year',
      'Allocation GOI Share (Cr)',
      'Allocation State Share (Cr)',
      'Allocation Total (Cr)',
      'SLSC GOI Share (Cr)',
      'SLSC State Share (Cr)',
      'SLSC Total (Cr)',
      'Sanction GOI Share (Cr)',
      'Sanction State Share (Cr)',
      'Sanction Total (Cr)',
      'BRO Released (Cr)',
      'DT Authorized (Cr)',
      'Bills Preferred Count',
      'Bills Preferred Amount (Cr)',
      'Oldest Bill Date',
      'Bills Cleared Count',
      'Bills Cleared Amount (Cr)',
      'Latest Bill Date',
      'Remark'
    ];
    ...
    filename = `central-sponsored-scheme_${financialYear}.xlsx`;
  }
    ...
    headers = [
      ...
      'Bills Preferred - Count',
      'Bills Preferred - Amount (Cr)',
      'Bills Preferred - Oldest Date',
      'Bills Cleared - Count',
      'Bills Cleared - Amount (Cr)',
      'Bills Cleared - Latest Date',
      'Pending Bills - Count',
      'Pending Bills - Amount (Cr)'
    ];

    dataRows = stateSchemeData.map((r, i) => [
      i + 1,
      r.name || '',
      r.hod || '',
      r.budgetEstimates ?? '',
    ...
    filename = `state-scheme_${financialYear}.xlsx`;
  }
    ...
    filename = `revenue_${financialYear}.xlsx`;
  }
  ...
  return (
    <div className="page-container" style={{ width: '100%', maxWidth: '100%', padding: '20px 24px' }}>
      {/* {importStatus && (
        <div style={{
          padding: '12px 16px',
          marginBottom: '16px',
      ...
      <div className="table-card" style={{ marginTop: '24px', width: '100%' }}>
        ...
        <div className="table-wrapper" style={{ border: '1px solid #d0d7de', borderRadius: '6px', overflow: 'auto', width: '100%' }}>
          {filterType === 'central-sponsored-scheme' && (
            <table style={{ width: '100%', minWidth: 'max-content', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th rowSpan="2" style={{ border: '1px solid #d0d7de', padding: '8px', background: '#f6f8fa' }}>Financial Year</th>
                  ...
                  financialRows.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.id || idx}>
                        ...
                        <td style={{ border: '1px solid #d0d7de', padding: '8px' }}>{row.financial_year || financialYear}</td>
