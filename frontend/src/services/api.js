...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response && error.response.status === 401) {
      // List of endpoints where 401 should NOT trigger logout redirect
      const excludedEndpoints = ['/nodal-officers', '/auth/change-password'];
      const isExcludedEndpoint = excludedEndpoints.some(endpoint => 
        error.config.url && error.config.url.includes(endpoint)
      );
      
      // Only redirect to login if it's not an excluded endpoint
